...
await page.goto('http://127.0.0.1:5230/?model=samples/MaterialsVariantsShoe.glb', { waitUntil: 'load' });
...
while (Date.now() - t0 < 700000) {
  ...
  st = await page.evaluate(() => { const s = window.viewer.getStats(); return [s.phase, Math.floor(s.samples)]; });
...
const snap = (label) => page.evaluate((label) => {
  ...
  return { label, phase: s.phase, samples: +v.pathTracer.samples.toFixed(2), low: v.pathTracer._lowResPathTracer.samples, eff: +s.effectiveSamples.toFixed(2), meanHist: +T.meanHistWeight.toFixed(2), dbg: { ...v.temporalDebug, lastReprojectAgo: Math.round(performance.now() - v.temporalDebug.lastReprojectAt) }, gslot: T.gslot, cur: T.cur, hasHistory: T.hasHistory,
    histObj: stat(T.hist, 3, objPred), histAll: stat(T.hist, 3), blendPrevObj: stat(T.blend[1 - T.cur], 3, objPred), gbufCls: stat(g, 3, (b, i) => b[i + 2] > 0), interacting: v.interacting, camKey: v.temporalCamKey.slice(0, 40) };
}, label);
console.log(JSON.stringify(await snap('static')));
await page.evaluate(async () => {
  const v = window.viewer; const pose = v.getCameraPose(); const target = pose.target.clone(); const start = pose.position.clone(); const up = pose.position.clone().set(0, 1, 0);
  v.beginInteraction(); const t0 = performance.now();
  await new Promise((res) => { const step = () => { const k = Math.min(1, (performance.now() - t0) / 350); const p = start.clone().sub(target).applyAxisAngle(up, (15 * Math.PI / 180) * k).add(target); v.syncCamera(p, target); if (k < 1) requestAnimationFrame(step); else res(); }; step(); });
  window.__endAt = performance.now(); v.endInteraction();
});
...
await page.screenshot({ path: '.captures/temporal_debug2.png' });
await browser.close();
