...
## 11. 배포 시점 성능 실측 (2026-08-21, Intel Arc 140T iGPU, 1120×856, 웜 캐시)

| 모델 | 삼각형 | 첫 샘플까지 | 128 spp 수렴 | 처리량 |
|---|---|---|---|---|
| 다이아 솔리테어 링 | 8.7k | 1.9 s | **8.1 s** | 15.9 spp/s |
| 스니커즈 | 22.7k | 2.5 s | **5.5 s** | 23.4 spp/s |
| 패브릭 체어(Sheen) | 39.9k | 4.4 s | **8.3 s** | 15.5 spp/s |
| 헬멧(PBR 텍스처) | 15.5k | 2.8 s | **8.5 s** | 15.1 spp/s |

측정: `tools/headless-bench.mjs`. 디노이저는 OIDN 시작 점검이 이 GPU 에서 à-trous 를 자동 선택.
내장 GPU 기준값이므로 외장 GPU(RTX 급)에서는 3~5배 빠를 것으로 추정(strahl 논문의 RTX 3080 ≈ M1 Max 2.3배 근거).
배포본: https://jhkim1543.github.io/vringon-viewer/ (재배포·커스텀 도메인은 docs/DEPLOY.md)

## 12. 최적성 감사와 개선 2건 (2026-08-21, 딥리서치 + 적대적 검증)

5개 각도(BVH 순회 / 셰이더 커널 / 디노이즈 / 광수송 / 우리 코드 리뷰)로 41건을 뽑아 8건을 적대적으로 검증(4건 생존, 4건 반박)한 뒤 속도 1 · 품질 1 을 골라 적용했다.

### 12.1 "현재 구현이 최선인가" 판정
**이미 최적이라 손댈 곳이 없는 축** (재확인됨): 샘플링(RANDOM_TYPE 2 = 계층화 + 픽셀별 블루노이즈), MIS, 러시안 룰렛 + 융합 파이어플라이 클램프, BVH 구축(SAH + leaf 1), near-child-first 순회(이미 splitAxis 기준으로 존재), tiles/samplesPerFrame/renderScale, 1회 컴파일 설계.
**BVH_STACK_DEPTH 30 은 더 내리면 안 된다** — maxDepth 28 기준 안전 하한이 29 이고, 순회 셰이더의 스택 *쓰기* 에는 경계 검사가 없어 넘치면 지오메트리가 조용히 사라진다.
**남은 것은 튜닝이 아니라 결함 2종**이었다(아래).

### 12.2 품질: stratifiedTexture 축 전치 버그 수정 (JS 전용, 재컴파일 0)
라이브러리가 `init( count=height=20, depth=width=bounces+transmissiveBounces+5 )` 로 잡는데, 셰이더는 `ivec2( effect id, sobolBounceIndex )` 로 읽는다 — **두 축이 뒤바뀌어 있다**.
기본값(5+10+5=20)에서는 우연히 맞지만 **조작 중 3/4 로 내리면 폭이 12** 가 되어 실제로 쓰이는 effect id 12~16
(GGX `rand2(12)`, 디퓨즈/클리어코트 `rand2(13)`, 시엔 `rand2(14)`, **BSDF 로브 선택 `rand(15)`**, 등방 산란 `rand2(16)`)과
우리 패치의 `rand4(12)`(shadowCatcher)·`rand(13)`(dispersion)이 전부 텍스처 밖을 읽는다.
범위 밖 `texelFetch` 는 0 을 돌려주므로 `rand` 가 `fract(0 + pixelSeed.r)` = **픽셀별 상수**로 굳는다.

수정: `prepareShaderOnce()` 에서 `stratifiedTexture.init` 을 감싸 폭을 상수 24 로 고정하고 높이에 `bounces+transmissiveBounces+5` 를 넘긴다.

| 측정 (조작 프리셋 3/4, 16 spp, 512 spp 레퍼런스 대비 RMSE) | 값 |
|---|---|
| 버그 상태(폭 12) | 12.888 |
| **수정 후(폭 24)** | **5.849** |
| | **오차 54.6 % 감소** |

부수 효과: `Bounces >= 9` 에서 세로축이 넘치던 문제도 함께 해결(Bounces 16 → 높이 31). 정지 화질 회귀 없음(링 1024spp 대비 상대 RMSE 29.78 % → 28.73 %).

### 12.3 속도: texelFetch1D 의 정수 나눗셈 제거 (`src/viewer/texelFetch.ts`)
three-mesh-bvh 의 `uTexelFetch1D`/`iTexelFetch1D`/`texelFetch1D` 세 함수가 페치마다 `index % width`, `index / width` 를 돈다.
GPU 에 정수 나눗셈 명령이 없고 텍스처 폭이 `ceil(sqrt(n))` 계열이라 2의 거듭제곱이 아니어서 shift/mask 강도 감소도 불가능하다.
BVH 순회는 내부 노드당 3회 호출되는 크리티컬 패스다. → float 역수 + ±1 보정으로 교체(읽는 텍셀은 동일).

**동등성 검증** (256 spp, 한 세션 교차):
| 비교 | RMSE |
|---|---|
| 대조군 A vs B (같은 셰이더 두 번) | 1.770 |
| 실험군 A vs C (패치 vs 원본) | 1.829 |

실험군이 대조군 노이즈 수준 안 — 이미지를 바꾸지 않는다. (`stableNoise` 는 업스트림 결함으로 결정적이지 않아 비트 비교는 불가능하며, 이 대조 설계가 대안이다.)

**속도 측정** (드리프트 상쇄를 위해 A/B 교대 후 인접 쌍 비교):
| 방법 | 결과 |
|---|---|
| 벽시계, 10쌍, 64 spp (ANGLE Vulkan) | 중앙값 +10.25 %, 7/10 양수 |
| GPU 타이머 쿼리, 5쌍 (ANGLE Vulkan) | 중앙값 +9.7 % 단축 (이봉형: +10 % 또는 −4.5 %) |
| **GPU 타이머 쿼리, 3쌍 (ANGLE D3D11 = 실제 배포 경로)** | **+17.7 / +22.8 / +14.9 %, 3/3 양수, 중앙값 +17.7 % 단축** |

**백엔드에 따라 이득이 다르다.** Vulkan 에서는 이봉형(드라이버 레지스터 할당에 따라 +10 % 또는 −4.5 %)이라 확정적이지 않지만,
**Chrome Windows 기본인 D3D11 에서는 3/3 모두 크게 양수**다 — fxc 가 정수 나눗셈을 float 역수 시퀀스로 더 비싸게 낮추기 때문으로 보인다.
사용자가 실제로 타는 경로에서 **약 15~23 % GPU 시간 단축**이며, 출력이 동일하고 롤백이 2줄이라 채택했다.

### 12.4 적용 후 최종 성능 (동일 조건, 128 spp 수렴)

| 모델 | 이전 | **이후** | 개선 |
|---|---|---|---|
| 다이아 솔리테어 링 | 8.1 s (15.9 spp/s) | **5.7 s (22.6)** | 30 % |
| 스니커즈 | 5.5 s (23.4) | **4.6 s (27.8)** | 16 % |
| 패브릭 체어 | 8.3 s (15.5) | **6.1 s (21.1)** | 27 % |
| 헬멧(PBR) | 8.5 s (15.1) | **6.8 s (18.9)** | 20 % |

4모델 전부 개선. 두 변경 중 속도 기여는 texelFetch 패치, 화질 기여는 stratified 축 교정이며
축 교정은 조작 중 샘플 품질을 올려 시간적 재투영이 재사용하는 프리뷰 데이터의 질도 함께 올린다.

### 12.5 반박되어 채택하지 않은 것 (근거 있는 기각)
- **환경맵 CDF 의 sin(θ) 누락 수정**: 무편향이지만 JS 와 GLSL 을 원자적으로 함께 고쳐야 하고(한쪽만 고치면 편향), 이득이 법선 방향 의존적이라 바닥에 놓인 신발·체어에는 거의 무이득.
- **픽셀별 유효 spp 를 à-trous σ_l 에 연결**: 진단은 옳으나 렌더 경로 상태 전진 규칙까지 함께 고쳐야 해 롤백 단위가 커진다.
- **`texture()` → `textureLod(...,0)` 전환**: 비트 동일성은 성립하나 ANGLE 이 불연속 루프 내 샘플을 이미 Lod0 변형으로 내보내 런타임 이득의 메커니즘이 없다(컴파일 시간만 이득).
- **`filterGlossyFactor` 의 isBelowSurface 가드 제거**: 순진하게 제거하면 다이아몬드가 확산체가 되고 수직 입사에서 NaN. 손대지 말 것.
- **BVH_STACK_DEPTH 추가 축소 / intersectsTriangle normalize 지연 / transmissiveBounces 재구축 제거**: 이득이 측정 잡음 수준이거나 이미 안전 하한.
