...
- **색 팔레트**: 따뜻한 중성 한 계열로 통일(다크 = 샴페인 `#C2A47E`, 라이트 = 브론즈 `#8A6A42`). 원본 VRINGON 토큰의 인디고+민트+주황+빨강 4계열은 제품 촬영 화면 위에서 산만하고 흔한 "AI 서비스" 인상을 준다. 상태는 색상(hue)이 아니라 밝기·채도로 구분한다. 값은 `styles.css` 상단 오버라이드 블록 한 곳에서만 바꾸고, 컴포넌트 CSS 는 계속 `var()` 만 쓴다.
